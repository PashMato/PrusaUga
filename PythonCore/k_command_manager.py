...
class KCommandManager:
    def __init__(self, lable, commands: list[np.ndarray], canvas_size: np.ndarray):
        self.label = lable
    ...
    def __add__(self, other):
        if type(other) == KCommandManager:
            assert not np.any(other.canvas_size != self.canvas_size), f"Error: CanvasSize must be the same for adding not `{other.canvas_size}`, `{self.canvas_size}`"
...
G1 Z{zpos}\n"""
            ...
            if (not is_printing and nis_printing) or (is_printing and n == 0):
            ...
            if is_printing:
                ...
                line_gcode = f"G1 X{e_pos[0]} Y{e_pos[1]} Z{zpos}\n"
            else:
                line_gcode = f"\nG1 F{50 * Data.PrintingSpeedFactor} Z{zpos - 4 / 6 * Data.PrintingSpeedFactor}\nG0 X{e_pos[0]} Y{e_pos[1]}\n"  # noqa

            code += line_gcode + gwait + penup_fore

        gcode = comments + meta_data + code
        gcode += f"\nG1 Z{zpos - 2}\nG0 X{-Data.ShiftX * Data.MMPerPoint} Y{-Data.ShiftY * Data.MMPerPoint}" # noqa
