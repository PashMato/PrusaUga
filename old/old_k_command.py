...
class KCommand:
    ...
    def __init__(self, start_position: np.array, end_position: np.array, should_print: bool):
        ...
        self.time: float = self.real_length / (Data.PrintingSpeedFactor * (200 * int(not self.is_printing)) + 200)
    ...
    def __str__(self):
        XYZ, is_z_shifted = self.to_k_code(Data.ShiftX, Data.ShiftY)
...
G1"""
        ...
        if not self.is_printing:
            massage = "\nG0" + massage[2:] + \
                      f"G1 Z{KCommand.ZPos - 100 * Data.PrintingSpeedFactor / 60} (wait a half second)\n"

            KCommand.ZPos += Data.PenUpFore * Data.PrintingSpeedFactor * Data.HeadSpeedRatio  # compute the start fore
            return massage + f"G1 Z{KCommand.ZPos} (wait a half second and give the suse a start fore)\n\n"
        else:
            massage += f"G1 Z{KCommand.ZPos - 100 * Data.PrintingSpeedFactor / 60} (wait a half second)\n" + \
    ...
    def to_k_code(self, shift_x: float = 0, shift_y: float = 0):
        ...
        return np.array([(self.end_position[1] - shift_x) * Data.MMPerPoint,  # noqa
                         ...
                         KCommand.ZPos], dtype=np.float), is_z_shifted  # noqa
