...
class NestedSvgParser:
    def __init__(self, file_name):
        ...
        message(f"Reading svg file ({file_name})", message_type="Info")
        self.parse()
    ...
    def separate_penups(self):
        eacc = []
        message("Separating pen-ups", message_type="Info")
    ...
    def sample_lines(self, scale=1):
        ...
        message("Sampling lines...", message_type="Info")
        for cur_path, trans in tqdm(self.lines):
            ...
            all_pos_trans *= np.array([1, -1])[:, np.newaxis]
            all_pos_trans += (self.size * np.array([0, 1]))[:, np.newaxis]
